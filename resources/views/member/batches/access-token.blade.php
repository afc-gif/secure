            ...
            @else
                <form method="POST" action="{{ route('member.access-token.store') }}" class="space-y-6" x-on:submit="loading = true; loadingText = 'Synchronizing Dashboard Privilege...'">
                    ...
                    <x-onboarding.secure-section
                        ...
                        description="Enter the VIP token issued by admin. The system verifies credential validity, batch synchronization constraints, and unique contract matching before secure access is granted."
                    />
                    <div>
