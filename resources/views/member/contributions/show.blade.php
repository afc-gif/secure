<x-dashboard.shell title="Contribution detail" eyebrow="Ownership Ledger">
    <section class="cca-card mx-auto max-w-3xl p-6 sm:p-8">
        ...
        <dl class="mt-8 grid gap-4 sm:grid-cols-2">
            ...
            <div class="rounded-lg border border-white/10 bg-white/[0.04] p-4"><dt class="text-xs uppercase tracking-[0.18em] text-slate-500">Submitted</dt><dd class="mt-2 text-xl font-black text-white">{{ $contribution->created_at->format('M d, Y') }}</dd></div>
            <div class="rounded-lg border border-white/10 bg-white/[0.04] p-4"><dt class="text-xs uppercase tracking-[0.18em] text-slate-500">Reviewed</dt><dd class="mt-2 text-xl font-black text-white">{{ $contribution->approved_at?->format('M d, Y') ?? 'Pending' }}</dd></div>
            <div class="rounded-lg border border-white/10 bg-white/[0.04] p-4"><dt class="text-xs uppercase tracking-[0.18em] text-slate-500">Admin Notes</dt><dd class="mt-2 text-sm leading-6 text-slate-300">{{ $contribution->admin_notes ?? 'No admin notes yet.' }}</dd></div>
        </dl>
