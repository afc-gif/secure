<x-dashboard.shell title="Generate access tokens" eyebrow="Token Registry">
    <section class="cca-card overflow-hidden">
        <div class="border-b border-white/10 px-4 py-4 sm:px-7 sm:py-5">
            <p class="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">Cooperative Access</p>
            <h2 class="mt-2 text-xl font-black text-white sm:text-2xl">Create secure batch tokens</h2>
        ...
        <form method="POST" action="{{ route('admin.tokens.store') }}" class="space-y-5 p-4 sm:space-y-6 sm:p-7">
            ...
            <div class="flex flex-col-reverse gap-3 border-t border-white/10 pt-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                <button class="cca-button w-full sm:w-auto">Generate Tokens</button>
            </div>
        </form>
    </section>
</x-dashboard.shell>
