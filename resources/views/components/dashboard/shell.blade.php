...
<x-app-layout>
    <div class="min-h-screen lg:grid lg:grid-cols-[17rem_1fr]">
        <aside class="border-b border-white/[0.07] bg-[#07080a]/90 px-5 py-5 backdrop-blur-xl lg:min-h-screen lg:border-b-0 lg:border-r lg:py-7">
            <div class="flex items-center justify-between lg:block">
                ...
                <span class="rounded-md border border-[#d8bf7a]/20 bg-[#d8bf7a]/10 px-3 py-1 text-xs font-semibold text-[#ead391] lg:mt-8 lg:inline-flex">{{ ucfirst($user->role) }}</span>
            </div>

            <nav class="mt-6 flex gap-2 overflow-x-auto lg:mt-10 lg:block lg:space-y-2">
                @foreach ($navigation as $item)
                    <a href="{{ $item['href'] }}" @class([
                        'block min-w-max rounded-md px-4 py-2.5 text-sm font-semibold transition',
                        'border border-[#d8bf7a]/25 bg-[#d8bf7a]/10 text-[#ead391] shadow-sm shadow-black/20' => $item['active'],
        ...
        <section class="min-w-0">
            <header class="sticky top-0 z-10 border-b border-white/[0.07] bg-[#08090b]/88 px-5 py-4 backdrop-blur-xl sm:px-8">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex h-10 w-10 items-center justify-center rounded-md border border-[#d8bf7a]/20 bg-[#d8bf7a]/10 text-sm font-black text-[#ead391]">{{ Str::of($user->name)->substr(0, 1)->upper() }}</div>
                    </div>
                </div>
            </header>
