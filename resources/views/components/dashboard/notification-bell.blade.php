...
<div class="relative">
    <details class="group">
        <summary class="flex h-11 w-11 cursor-pointer list-none items-center justify-center rounded-lg border border-white/10 bg-white/[0.06] text-[#d4af62] transition hover:bg-white/[0.1]">
            <span class="sr-only">Notifications</span>
            <span class="text-lg">!</span>
            @if ($notifications->count())
                <span class="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-emerald-300 px-1 text-[10px] font-black text-[#08100c]">{{ $notifications->count() }}</span>
            @endif
        </summary>
        <div class="absolute right-0 z-20 mt-3 w-72 rounded-lg border border-white/10 bg-[#0b1110] p-3 shadow-2xl">
